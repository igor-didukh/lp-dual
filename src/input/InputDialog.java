...
public class InputDialog extends JDialog implements ActionListener {
	...
	private void makeName() {
		...
		String name;
		if (mode == 0)
			name = Common.makePolynomName(c, "x", " --> " + dir);
		else
			name = Common.makePolynomName(c, "x", dir) + " " + Common.clip(txtC.getDouble());
	...
	public void actionPerformed(ActionEvent ae) {
		if (ae.getActionCommand() == SAVE) {
			...
			if (zeroCoefsCount == amount)
				...
				String dir = (String) comboDirection.getSelectedItem();
				if (mode == 0)
					MainWindow.fnFromDialog = new Function(c, "x", dir);
				else
					MainWindow.fnFromDialog = new Constraint(c, "x", dir, txtC.getDouble());
				dispose();
			}
		} 
		else
