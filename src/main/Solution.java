...
public class Solution {
	...
	public Solution (Problem problem, String outputMode) {
		fn = problem.getF();
		cons = problem.getConstraints();
		vars = problem.getRestrictions();
		
		switch (outputMode) {
		case Common.TXT:
			out = new OutTXT( fn.getLetter() );
			break;
			
		case Common.HTML:
			out = new OutHTML( fn.getLetter() );
			break;
		}
	}
	...
	private void printOptimalSolution() {
		...
		String s = "";
		for (int j = 1; j <= n; j++)
			s += String.format("%s[%d] = %s; ", fn.getLetter(), j, Common.clip( res[j] ) );
		out.printP(s);
